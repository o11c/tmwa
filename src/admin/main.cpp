...
sint32 loginserverport = 6900;    // Port of login-server
char loginserveradminpassword[24] = "admin";    // Administration password
sint32 passenc = 2;               // Encoding type of the password
...
sint32 login_fd;
IP_Address login_ip;
sint32 bytes_to_read = 0;         // flag to know if we waiting bytes from login-server
char command_[1024];
char parameters[1024];
sint32 list_first, list_last, list_type, list_count;  // parameter to display a list of accounts
sint32 already_exit_function = 0; // sometimes, the exit function is called twice... so, don't log twice the message

//------------------------------
// Writing function of logs file
//------------------------------
static sint32 ladmin_log(const char *fmt, ...) __attribute__((format(printf, 1, 2)));
sint32 ladmin_log(const char *fmt, ...)
{
    FILE *logfp;
    va_list ap;
    struct timeval tv;
    ...
    if (logfp)
    {
        ...
        else
        {
            ...
            sprintf(tmpstr + strlen(tmpstr), ".%03d: %s",
                    static_cast<sint32>(tv.tv_usec / 1000), fmt);
...
//---------------------------------------------
static const char *makeordinal(sint32 number)
{
...
//-----------------------------------------------------------------------------------------
static sint32 verify_accountname(char *account_name)
{
    sint32 i;
...
//----------------------------------
static sint32 typepasswd(char *password)
{
...
//------------------------------------------------------------------------------------
static sint32 verify_password(char *password)
{
    sint32 i;
    ...
    {
        printf("Account name is too short. Please input an password of 4-23 bytes.\n");
        ladmin_log("Account name is too short. Please input an password of 4-23 bytes.\n");
        return 0;
    }
...
//-----------------------------------------------------------------
static sint32 check_command(char *command)
{
// help
...
//-----------------------------
static sint32 addaccount(char *param, sint32 emailflag)
{
...
//---------------------------------------------------------------------------------
static sint32 banaddaccount(char *param)
{
    char name[1023], modif[1023];
    sint32 year, month, day, hour, minute, second;
    char *p_modif;
    sint32 value, i;
...
//-----------------------------------------------------------------------
static sint32 bansetaccountsub(char *name, const char *date, const char *time_)
{
    sint32 year, month, day, hour, minute, second;
    ...
    else
    {
        ...
        if (ban_until_time == -1)
        {
            printf("Invalid date.\n");
            printf("Please input a date and a time (format: yyyy/mm/dd hh:mm:ss).\n");
            printf("You can input 0 instead of if you use 'banset' command.\n");
...
//---------------------------------------------------------------------
static sint32 banaccount(char *param)
{
...
//------------------------------------------------------------------------
static sint32 bansetaccount(char *param)
{
    char name[1023], date[1023], time_[1023];
...
//-------------------------------------------------
static sint32 unbanaccount(char *param)
{
    char name[1023];
...
//---------------------------------------------------------
static sint32 checkaccount(char *param)
{
...
//------------------------------------------------
static sint32 delaccount(char *param)
{
    char name[1023];
    char letter;
    char confirm[1023];
...
//----------------------------------------------------------
static sint32 changeemail(char *param)
{
...
//-----------------------------------------------------
static sint32 getlogincount(void)
{
...
//----------------------------------------------------------
static sint32 changegmlevel(char *param)
{
    char name[1023];
    sint32 GM_level;
...
//---------------------------------------------
static sint32 idaccount(char *param)
{
    char name[1023];
...
//----------------------------------------------------------------------------
static sint32 infoaccount(sint32 account_id)
{
    if (account_id < 0)
    {
...
//---------------------------------------
static sint32 sendbroadcast(sint16 type, char *message)
{
    if (strlen(message) == 0)
    {
...
//--------------------------------------------------------
static sint32 listaccount(char *param, sint32 type)
{
//sint32 list_first, list_last, list_type; // parameter to display a list of accounts
    sint32 i;
...
//--------------------------------------------------------
static sint32 itemfrob(char *param)
{
    sint32 source_id, dest_id;
...
//--------------------------------------------
static sint32 changememo(char *param)
{
...
//-----------------------------------------------
static sint32 nameaccount(sint32 id)
{
    if (id < 0)
    {
        printf("Please input a positive value for the id.\n");
        ladmin_log("Negative id given to search an account name ('name' command).\n");
        return 136;
    }
...
// (Note: never send back a password with login-server!! security of passwords)
//------------------------------------------
static sint32 changepasswd(char *param)
{
    char name[1023], password[1023];
...
//-----------------------------------------------------
static sint32 changesex(char *param)
{
...
//-------------------------------------------------------------------------
static sint32 changestatesub(char *name, sint32 state, const char *error_message7)
{
...
//-------------------------------------------------------
static sint32 changestate(char *param)
{
...
//-------------------------------------------
static sint32 unblockaccount(char *param)
{
    char name[1023];
...
// Sub-function: Asking to unblock an account
//-------------------------------------------
static sint32 blockaccount(char *param)
{
    char name[1023];
...
//---------------------------------------------------------------------
static sint32 timeaddaccount(char *param)
{
    char name[1023], modif[1023];
    sint32 year, month, day, hour, minute, second;
    char *p_modif;
...
//-------------------------------------------------
static sint32 timesetaccount(char *param)
{
    char name[1023], date[1023], time_[1023];
    sint32 year, month, day, hour, minute, second;
...
//------------------------------------------------------------------------------
static sint32 whoaccount(char *param)
{
    char name[1023];
...
//--------------------------------------------------------
static sint32 checkloginversion(void)
{
...
//-------------------------------------------------------------
static void parse_fromlogin(sint32 fd)
{
    if (session[fd]->eof)
    {
                    ...
                    for (i = 4; i < RFIFOW(fd, 2); i += 38)
                    {
                            ...
                            || (list_type == 4 && RFIFOL(fd, i + 34) == 0))
                        {
                            ...
                            if (RFIFOB(fd, i + 4) == 0)
                                ...
                                printf("%2d ", static_cast<sint32>(RFIFOB(fd, i + 4)));
                            printf("%-24s", userid);
                            if (RFIFOB(fd, i + 29) == 0)
                                printf("%-5s ", "Femal");
                            ...
                            switch (RFIFOL(fd, i + 34))
                            {
                                ...
                                case 5:
                                    ...
                                    break;
                                case 6:
                                    printf("%-27s\n", "Your client is too old"); // Your Game's EXE file is not the latest version
                                    break;
                                case 7:
//                                     printf("%-27s\n", "Banishment or");
            ...
            case 0x7937:       // answer of the change of an account state
                ...
                {
                    char tmpstr[256];
                    sprintf(tmpstr, "Account [%s] state successfully changed in [",
                             RFIFOP(fd, 6));
                    switch (RFIFOL(fd, 30))
                    {
                        case 0:
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            strcat(tmpstr, "6: Your client is not the latest version");
                            break;
                        case 7:
            ...
            case 0x794f:       // answer of a broadcast
                if (RFIFOREST(fd) < 4)
                    return;
                if (RFIFOW(fd, 2) == static_cast<uint16>(-1))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            printf(" Id:     %d (GM level %d)\n",
                                    RFIFOL(fd, 2), static_cast<sint32>(RFIFOB(fd, 6)));
                        }
                        printf(" Name:   '%s'\n", userid);
                        if (RFIFOB(fd, 31) == 0)
                            printf(" Sex:    Female\n");
                        ...
                        {
                            case 0:
                                printf(" State: 0 [Account OK]\n");
                                break;
                            case 1:
                                printf(" State: 1 [Unregistered ID]\n");
                                break;
                            case 2:
                                printf(" State: 2 [Incorrect Password]\n");
                                break;
                            case 3:
                                printf(" State: 3 [This ID is expired]\n");
                                break;
                            case 4:
                                printf(" State: 4 [Rejected from Server]\n");
                                break;
                            case 5:
                                printf(" State: 5 [You have been blocked by the GM Team]\n");
                                break;
                            case 6:
                                printf(" State: 6 [Your Game's EXE file is not the latest version]\n");
                                break;
                            case 7:
                                printf(" State: 7 [You are Prohibited to log in until %s]\n",
                                    error_message);
                                break;
                            case 8:
                                printf(" State: 8 [Server is jammed due to over populated]\n");
                                break;
                            case 9:
                                printf(" State: 9 [No MSG]\n");
                                break;
                            default:   // 100
                                printf(" State: %d [This ID is totally erased]\n",
                                        RFIFOL(fd, 36));
                                break;
                        }
                        if (ban_until_time == 0)
...
//------------------------------------
static sint32 Connect_login_server(void)
{
...
//-----------------------------------
static sint32 ladmin_config_read(const char *cfgName)
{
            ...
            if (strcasecmp(w1, "login_ip") == 0)
            {
                ...
                if (h != NULL)
                {
                    Iprintf("Login server IP address: %s -> %hhu.%hhu.%hhu.%hhu\n",
                             w2, static_cast<uint8>(h->h_addr[0]),
                            static_cast<uint8>(h->h_addr[1]),
                            static_cast<uint8>(h->h_addr[2]),
                            static_cast<uint8>(h->h_addr[3]));
                    sprintf(loginserverip, "%hhu.%hhu.%hhu.%hhu",
                            static_cast<uint8>(h->h_addr[0]),
                            static_cast<uint8>(h->h_addr[1]),
                            static_cast<uint8>(h->h_addr[2]),
                            static_cast<uint8>(h->h_addr[3]));
                }
