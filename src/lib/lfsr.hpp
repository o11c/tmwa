...
uint64_t lfsr_next(uint64_t last, int bits, bool allow_zero_state) __attribute__((pure));
inline uint64_t lfsr_next(uint64_t last, int bits, bool allow_zero_state)
{
    if (2 <= bits && bits <= 64)
        return lfsr_next_internal(last, bits, allow_zero_state);
    abort();
